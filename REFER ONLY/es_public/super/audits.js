(function () {
  ...
  if (!auth || !auth.requireAuthOrRedirect('super/audits.html')) return;
  ...
  function wireEvents(bid) {
    if ($('audits-bot') && !$('audits-bot').value) {
      $('audits-bot').value = bid || '';
      $('audits-bot').placeholder = bid || 'All bots';
    }
  ...
  function boot() {
    ...
    nav
      .mountPage({
        active: 'audits',
        title: 'Audits',
        subtitle: 'Change history · ' + bot.name + ' (Bot ID ' + bot.id + ')',
        bid: bid,
      })
      .then(function () {
        wireEvents(bid);

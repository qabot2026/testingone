...
/** Live-agent handoffs from all bots — SHEETS_AGENT_TAB on Railway. */
function agentTabName() {
  if (AGENT_TAB) return AGENT_TAB;
  return dashboardTabName();
}

function quoteTabForRange(tab) {
...
function tabRange(tab, rangePart) {
  return quoteTabForRange(tab) + '!' + rangePart;
}

async function listSpreadsheetTabTitles() {
...
/** Create spreadsheet tab if missing (called from Supersetting / first sync). */
async function ensureTabExists(tab) {
  const name = String(tab || '').trim();
  if (!name || !isConfigured()) return false;
  const titles = await listSpreadsheetTabTitles();
  if (titles.includes(name)) return true;
  const client = getSheetsClient();
...
async function writeConvLinkForRowOnTab(tab, rowNumber1Based, sessionId) {
  ...
  try {
    await client.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range: tabRange(tab, 'A' + rowNumber1Based),
    ...
    logSheetError('writeConvLinkForRowOnTab', err);
    if (plain && plain !== cell) {
      try {
        await client.spreadsheets.values.update({
          spreadsheetId: SPREADSHEET_ID,
        ...
        logSheetError('writeConvLinkForRowOnTab/plain', err2);
      }
    }
    return false;
  }
...
async function writeConvLinkForRow(rowNumber1Based, sessionId) {
  return writeConvLinkForRowOnTab(tabName(), rowNumber1Based, sessionId);
}
...
async function ensureSheetDateColumnsFormatOnTab(tab) {
  if (!isConfigured() || !tab) return;
  ...
  try {
    ...
    logSheetError('ensureSheetDateColumnsFormatOnTab', err);
  }
}

async function ensureSheetDateColumnsFormat() {
  return ensureSheetDateColumnsFormatOnTab(tabName());
}
...
async function ensureHeaderRowOnTab(tab, headers) {
  ...
  const headerRange = tabRange(tab, 'A1:' + columnToLetter(headers.length) + '1');
  try {
    const res = await client.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: headerRange,
...
async function updateRowOnTab(tab, rowNumber1Based, values) {
  ...
  const range = tabRange(tab, 'A' + rowNumber1Based + ':' + endCol + rowNumber1Based);
  try {
    await client.spreadsheets.values.update({
      spreadsheetId: SPREADSHEET_ID,
      range,
 ...
 */
async function listSheetMobilesOnTab(tab, excludeRowNumber1Based) {
  if (!isConfigured() || !tab) return [];
  const client = getSheetsClient();
  if (!client) return [];
  ...
  try {
    ...
    const res = await client.spreadsheets.values.get({
      spreadsheetId: SPREADSHEET_ID,
      range: tabRange(tab, mobileCol + '2:' + mobileCol),
    });
    const rows = res.data.values || [];
    const out = [];
    ...
    logSheetError('listSheetMobilesOnTab', err);
    return [];
  }
}

async function listSheetMobiles(excludeRowNumber1Based) {
  return listSheetMobilesOnTab(tabName(), excludeRowNumber1Based);
}
 ...
 */
async function fetchConversationGrid(tabOverride) {
  if (!isConfigured()) {
  ...
  const tab = String(tabOverride || '').trim() || tabName();
  await ensureTabExists(tab);
  const meta = await client.spreadsheets.get({
    spreadsheetId: SPREADSHEET_ID,

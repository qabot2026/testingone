...
function scheduleTranscriptCloudSync(sessionId) {
  try {
    require('./transcript-sync').scheduleSync(sessionId);
  } catch (e) {
    console.warn('[transcript→cloud]', e.message);
  }
}
...
function appendTurn(sessionId, role, text, meta, options) {
  ...
  if (normalizedRole === 'bot') {
    const lastBot = doc.turns.length ? doc.turns[doc.turns.length - 1] : null;
    if (
      lastBot &&
      normalizeRole(lastBot.role) === 'bot' &&
      String(lastBot.text || '').trim() === t
    ) {
      if (explicitId && !lastBot.id) lastBot.id = explicitId;
      return lastBot;
    }
  }
  ...
  saveIndex(index);
  scheduleTranscriptCloudSync(sid);
...
function mergeSessionMeta(sessionId, partial, options) {
  ...
  if (!prev.sheetBotId) {
    ...
    next.sheetBotId = botSheetTabs.resolveSheetBotIdFromMeta(next);
  } else {
    next.sheetBotId = prev.sheetBotId;
  }
  doc.meta = next;
  fs.writeFileSync(file, JSON.stringify(doc, null, 2), 'utf8');
  scheduleTranscriptCloudSync(sid);
...
function getSheetRowForTab(sessionId, tab) {
  const sid = String(sessionId || '').trim();
  const tabKey = String(tab || '').trim();
  if (!sid || !tabKey) return null;
  const doc = getSessionDoc(sid);
  if (doc.sheetRows && doc.sheetRows[tabKey] >= 2) {
    return Number(doc.sheetRows[tabKey]);
  }
  return null;
}

function setSheetRowForTab(sessionId, tab, rowNum) {
  const sid = String(sessionId || '').trim();
  const tabKey = String(tab || '').trim();
  if (!sid || !tabKey || !rowNum) return;
  const doc = getSessionDoc(sid);
  if (!doc.sheetRows || typeof doc.sheetRows !== 'object') {
    doc.sheetRows = {};
  }
  doc.sheetRows[tabKey] = rowNum;
  doc.sheetRow = rowNum;
  ensureDir();
  fs.writeFileSync(
    sessionPath(sid),
...
function botDisplayTextFromDialogflowResult(result) {
  ...
  if (reply && reply !== 'No response.') {
    if (messageSyntax.hasMessageSyntax(reply)) {
      return messageSyntax.stripToPlainText(reply);
    }
    return reply;
  }
...
function logDialogflowExchange(sessionId, userMessage, result, options) {
  ...
  const botText = botDisplayTextFromDialogflowResult(result);
  const botMeta = richMetaFromDialogflowResult(result);
  if (botText || botMeta) {
    appendTurn(
      sid,
      'bot',
      botText || '(Rich content)',
      botMeta,
      noSchedule
    );
  }
  ...
  if (result.intentIsFallback) {
    mergeSessionMeta(sid, { fallback: 'yes', lastIntent: 'fallback', intentIsFallback: true }, { scheduleSheet: false });
  } else if (result.intent) {
    mergeSessionMeta(
      sid,
      { lastIntent: String(result.intent).trim(), intentIsFallback: false },
      { scheduleSheet: false }
    );
  }

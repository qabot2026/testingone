...
async function resolveRepeatedUserLabel(doc) {
  ...
  const botTab = botSheetTabs.resolveConversationTabForMeta(meta);
  const excludeRow =
    (botTab && doc.sheetRows && Number(doc.sheetRows[botTab]) >= 2
      ? Number(doc.sheetRows[botTab])
      : null) ||
    (doc.sheetRow && Number(doc.sheetRow) >= 2 ? Number(doc.sheetRow) : null);
  const existing = botTab
    ? await sheets.listSheetMobilesOnTab(botTab, excludeRow)
    : await sheets.listSheetMobiles(excludeRow);
...
function metaFromClientBody(body) {
  ...
  pick(['sitePreset', 'site_preset'], 'sitePreset');
  pick(['botId', 'bot_id'], 'botId');
...
async function runSheetSync(sessionId) {
  ...
  const botTab = botSheetTabs.resolveConversationTabForMeta(doc.meta || {});
  if (!botTab) {
    return { skipped: true, reason: 'no_bot_tab' };
  }

  const values = buildRowValues(doc);
  await sheets.ensureTabExists(botTab);
  await sheets.ensureHeaderRowOnTab(botTab, SHEET_HEADERS);
  await sheets.ensureSheetDateColumnsFormatOnTab(botTab);

  const writeRow = async (rowNum) => {
    await sheets.updateRowOnTab(botTab, rowNum, values);
    await sheets.writeConvLinkForRowOnTab(botTab, rowNum, sid);
  };

  let sheetRow = chatTranscript.getSheetRowForTab(sid, botTab);
  if (!(sheetRow >= 2)) {
    ...
    if (found && found.rowNumber >= 2) {
      ...
      chatTranscript.setSheetRowForTab(sid, botTab, sheetRow);
    }
  }
  ...
  if (sheetRow >= 2) {
    ...
    const result = {
      ...
      updated: sheetRow,
      tab: botTab,
      convLink: values[0] ? 'yes' : 'no',
    };
    try {
      const leadRunner = require('./lead-notifications-runner');
      leadRunner
        .onConversationSynced(sid, doc.meta, result)
        .catch((e) => console.warn('[lead-notifications] instant:', e.message));
    } catch (e) {
      /* optional module */
    }
    return result;
  ...
  const rowNum = await sheets.appendRowValuesOnTab(botTab, values);
  if (!rowNum) {
    console.warn('[conversation-sheet] append returned no row for', sid, 'tab', botTab);
    return { ok: false, error: 'append_failed', tab: botTab };
  }

  await sheets.writeConvLinkForRowOnTab(botTab, rowNum, sid);
  chatTranscript.setSheetRowForTab(sid, botTab, rowNum);

  const result = { ok: true, appended: rowNum, tab: botTab, convLink: values[0] ? 'yes' : 'no' };
  try {
    const leadRunner = require('./lead-notifications-runner');
    leadRunner
      .onConversationSynced(sid, doc.meta, result)

...
const BOT_CONFIG_MANIFEST = 'es_public/client-based/bot-configs/manifest.json';
...
function pathsForBot(bot) {
  ...
  return [
    `es_public/client-based/bot-configs/${bot.sitePreset}.config.js`,
...
async function collectBotAssetChanges() {
  ...
  changes.push({
    path: `es_private/client-based/data/${BOT_SYNC_MANIFEST}`,
...
async function buildGithubChangeSet() {
  ...
  for (const fileName of SYNC_FILES) {
    ...
    if (useGithub()) {
      githubChanges.push({
        path: `es_private/client-based/data/${fileName}`,
        content,
      });
    }
  }
...
async function readRemoteRegistryTimestamp() {
  ...
  if (useGithub()) {
    try {
      const [owner, repo] = GITHUB_REPO.split('/');
      const url = `https://api.github.com/repos/${owner}/${repo}/contents/es_private/client-based/data/bot-registry.json?ref=${encodeURIComponent(GITHUB_BRANCH)}`;
      const res = await fetch(url, { headers: githubHeaders() });
      if (res.ok) {
        const meta = await res.json();
        const remote = Buffer.from(meta.content || '', 'base64').toString('utf8');
...
function restoreBotAssetsFromGit() {
  try {
    ...
    execSync(
      'git restore --source=origin/main es_public/client-based/bot-configs es_public/client-based/bot-settings',

...
function resolveAssistantName(botId, sitePreset) {
  const id = String(botId || '').trim();
  if (id) {
    const project = sitePresetsStore.resolveProject(id);
    if (project && project.name) return String(project.name).trim();
  }
  const preset = String(sitePreset || '').trim();
  if (preset) {
    const byPreset = sitePresetsStore
      .listProjects()
      .find((p) => p.sitePreset === preset);
    if (byPreset && byPreset.name) return String(byPreset.name).trim();
    const overrides = sitePresetsStore.getPublicOverrides();
    const cfg = overrides[preset];
    const persona =
      cfg &&
      cfg.common &&
      cfg.common.botPersona &&
      cfg.common.botPersona.label;
    if (persona) return String(persona).trim();
    const title =
      cfg && cfg.common && cfg.common.header && cfg.common.header.title;
...
function transcriptToMeta(entry) {
  ...
  const botId = entry.botId || entry.bot_id || '';
  const sitePreset = entry.sitePreset || entry.site_preset || '';
  return {
    ...
    sitePreset,
    assistant:
      entry.assistant || resolveAssistantName(botId, sitePreset) || '',
...
function readTranscriptEntry(doc, sessionIdFromFile) {
  const m = doc.meta || {};
  const botId = m.botId || m.bot_id || '';
  const sitePreset = m.sitePreset || m.site_preset || '';
  return {
    ...
    botId,
    sitePreset,
    assistant: resolveAssistantName(botId, sitePreset),
...
function metaFromGcsCustom(fileMetadata) {
  const md = fileMetadata || {};
  const custom = md.metadata || {};
  const botId = custom.bot_id || custom.botId || '';
  const sitePreset = custom.site_preset || custom.sitePreset || '';
  return {
    ...
    files.forEach((file) => {
      try {
        ...
        const seed = {
          ...
          tag: m.tag || m.upload_tag || '',
          botId: m.botId || m.bot_id || '',
          sitePreset: m.sitePreset || m.site_preset || '',
          assistant: resolveAssistantName(
            m.botId || m.bot_id,
            m.sitePreset || m.site_preset
          ),
          storage_folder: m.storage_folder || '',
  ...
  (records || []).forEach((rec) => {
    ...
    if (!g) {
      ...
      g = {
        ...
        bot_id: rec.botId || '',
        site_preset: rec.sitePreset || '',
        assistant: rec.assistant || resolveAssistantName(rec.botId, rec.sitePreset),
    ...
    g.files.push({
      ...
      assistant: rec.assistant || g.assistant || '',
...
function applyMetaToGroup(g, meta) {
  ...
  if (meta.botId && !g.bot_id) g.bot_id = meta.botId;
  if (meta.sitePreset && !g.site_preset) g.site_preset = meta.sitePreset;
  if (meta.assistant && !g.assistant) g.assistant = meta.assistant;
  if (
    !g.assistant &&
    (meta.botId || meta.sitePreset || g.bot_id || g.site_preset)
  ) {
    g.assistant = resolveAssistantName(
      meta.botId || g.bot_id,
      meta.sitePreset || g.site_preset
    );
  }
  ...
  for (const file of files) {
    ...
    if (!g) {
      ...
      g = {
        ...
        bot_id: (resolved && resolved.botId) || '',
        site_preset: (resolved && resolved.sitePreset) || '',
        assistant:
          (resolved && resolved.assistant) ||
          resolveAssistantName(
            resolved && resolved.botId,
            resolved && resolved.sitePreset
          ) ||
          '',

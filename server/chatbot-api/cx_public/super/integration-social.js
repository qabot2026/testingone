(function () {
  ...
  function setIntro() {
    ...
    el.textContent =
      ...
      ' — WhatsApp, Instagram aur Facebook ki alag-alag settings.';
  }
  ...
  function renderChannelList() {
    ...
    root.innerHTML = CHANNELS.map(function (channelId) {
      ...
      return (
        '<article class="social-channel social-channel--' +
        esc(channelId) +
        (isOpen ? ' is-open' : '') +
        '" data-channel="' +
        esc(channelId) +
        ...
        '">' +
        '<span class="' +
        channelIconClass(channelId) +
        '">' +
        channelIconSvg(channelId) +
        '</span>' +
        ...
        esc(badge.text) +
        '</span>' +
        '<span class="social-channel__chev" aria-hidden="true"><svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round"><path d="M6 9l6 6 6-6"/></svg></span>' +
        '</button>' +

...
function liveAgentAllowDialogflowForUserText_() {
    ...
    return liveAgentCachedConvStatus === "waiting";
}

/**
 * Block native df-messenger send to Dialogflow; route text to live agent inbox only.
 * @param {Event | null | undefined} event
...
function ensureAssistantTranscriptCapturedForDfEvent_(event) {
    ...
    if (structuredTurns.length) {
        ...
        const seqAfterStructured =
            ...
            && Number.isFinite(afterStructured.chat_transcript_seq)
...
function captureDomBotTranscriptLines_(dfMessenger) {
    ...
    try {
        ...
        const fresh = lines.filter((line) => {
            ...
            if (plainTextRedundantWithAssistantRow_(line, lastRow)) {
                return false;
            }
            return true;
        });
 ...
 * @returns {{ role?: string, text?: string, rich?: Record<string, unknown>, at?: number } | null}
 */
function lastAssistantTranscriptRow_(transcript) {
    const tr =
        Array.isArray(transcript) && transcript.length
            ? transcript
            : (() => {
                  const prev = readStoredClientContext();
                  return prev.chat_transcript && Array.isArray(prev.chat_transcript)
                      ? prev.chat_transcript
                      : [];
              })();
 ...
 * @param {unknown[] | undefined} [transcript]
 */
function assistantTranscriptCapturedWithinMs_(ms, transcript) {
    const last = lastAssistantTranscriptRow_(transcript);
    if (!last || typeof last.at !== "number" || !Number.isFinite(last.at)) {
        return false;
    }
    return Date.now() - last.at < ms;
}

/**
 * Plain line already represented by the last assistant row (text and/or CX `rich` chips).
 *
 * @param {string} plain
 * @param {{ role?: string, text?: string, rich?: Record<string, unknown> } | null} row
 */
function plainTextRedundantWithAssistantRow_(plain, row) {
    if (!row || row.role !== "assistant") {
        return false;
    }
    const norm = normalizeChatTranscriptCompareText_(plain);
    if (!norm) {
        return false;
    }
    const lastText = normalizeChatTranscriptCompareText_(row.text || "");
    if (
        lastText
        && (lastText === norm || lastText.includes(norm) || norm.includes(lastText))
    ) {
        return true;
    }
    const rich = row.rich;
    if (rich && typeof rich === "object" && !Array.isArray(rich)) {
        ...
        for (let li = 0; li < labels.length; li += 1) {
            const ln = normalizeChatTranscriptCompareText_(labels[li]);
            if (ln && (ln === norm || norm.includes(ln) || ln.includes(norm))) {
                return true;
            }
        }
        const joined = normalizeChatTranscriptCompareText_(labels.join(" "));
        if (joined && (joined === norm || norm.includes(joined) || joined.includes(norm))) {
            return true;
        }
    }
    return false;
    ...
    try {
        ...
        for (let i = 0; i < batch.length; i += 1) {
            ...
            const sameAsLast =
                ...
                && last.role === "assistant"
                && normalizeChatTranscriptCompareText_(last.text) === normalizeChatTranscriptCompareText_(text);
            if (sameAsLast || plainTextRedundantWithAssistantRow_(text, last)) {
                continue;
            }
            if (
                isWidgetFormThankYouSummaryLine_(text)
...
/** @param {unknown} raw */
function normalizePayloadArrayForWeb_(raw) {
    if (!Array.isArray(raw)) {
        return null;
    }
    return raw.map((item) => normalizePayloadObjectForWeb_(item) || item);
}
...
function collectRichContentChipPrompts_(node, out, seen, depth) {
    ...
    const maybeArray = normalizePayloadArrayForWeb_(node);
    if (maybeArray) {
        for (const item of maybeArray) {
            collectRichContentChipPrompts_(item, out, seen, depth + 1);
        }
        return;
    }
    ...
    const queryResult = normalized.queryResult;
    if (queryResult && typeof queryResult === "object" && !Array.isArray(queryResult)) {
        const responseMessages = /** @type {{ responseMessages?: unknown[] }} */ (queryResult).responseMessages;
        if (Array.isArray(responseMessages)) {
            collectRichContentChipPrompts_(responseMessages, out, seen, depth + 1);
        }
    }
...
/* User persona row (SVG badge embedded in markdown bot bubble) — strip bubble chrome + right-align. */
.message.bot-message.markdown:has(img[src*="dfchat-persona-user|"]),
.message.bot-message.markdown:has(img[src*="dfchat-persona-user%7C"]),
.message.bot-message:has(img[src*="dfchat-persona-user|"]):not(.markdown),
.message.bot-message:has(img[src*="dfchat-persona-user%7C"]):not(.markdown) {
...
df-markdown-message.dfchat-user-persona-md-host,
...
.message-stack:has(img[src*="dfchat-persona-user|"]),
.message-stack:has(img[src*="dfchat-persona-user%7C"]) {
  align-self: flex-end !important;
  margin-left: auto !important;
  margin-right: 0 !important;
  background: transparent !important;
...
/**
 * Strip the default bot-message bubble around a user persona SVG badge ({@link createPersonaBadgeMarkdown})
 * and right-align it. The badge is rendered via `renderCustomText(md, true)` (markdown lane) so its `<img>`
 * actually paints — but that places it inside `.message.bot-message.markdown`, which has bot padding/bg.
 * We walk the composed parents to make stacks/entries/markdown-host/bubble transparent and align to flex-end.
 * Bot persona styling stays untouched (`applyBotEmojiPersonaCaptionChrome` is its own function).
 * @param {HTMLImageElement} imageNode
 */
function applyUserPersonaCaptionChrome_(imageNode) {
    if (!imageNode || getPersonaType(imageNode) !== "user") {
        return;
    }
    ...
    for (let i = 0; el && i < 28; i += 1) {
        ...
        try {
            if (isMarkdownMessageHost) {
                try {
                    el.classList.add("dfchat-user-persona-md-host");
                } catch (eMd) {
                    /* ignore */
                }
            ...
            if (isBotEntry) {
                try {
                    el.classList.add("dfchat-user-persona-entry");
                } catch (eE) {
                    /* ignore */
                }
                el.style.setProperty("align-self", "flex-end", "important");
...
function stylePersonaContainer(container, imageNode, personaType) {
    ...
    while (current && current !== document.body && depth < 3) {
        ...
        if (personaType === "user") {
            ...
            {
                const gap = Math.max(0, Math.min(64, BOT_PERSONA_CONFIG.gapBelowAssistantPx ?? 4));
